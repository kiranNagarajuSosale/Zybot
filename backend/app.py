...
from runtime_tracer import get_runtime_trace_log
#from ingest import ingest_github_repo, ingest_repo_from_zip
...
app = FastAPI(
    ...
    description="LangChain + Gemini Hybrid Retrieval Bot with Eval, Debug, Multi-repo, Active Learning",
    version="1.1"
)
...
async def chat(query: Query):
    chain = load_chain(
        role=query.role,
        page_url=query.page_url or None,
        element_descriptor=query.element_descriptor or None,
        trace_path=query.trace_path or None
    )
...
async def chat_debug(query: Query):
    ...
    result = chain.invoke({"question": query.question, "context": ""})
    docs = chain.retriever.get_relevant_documents(query.question)
